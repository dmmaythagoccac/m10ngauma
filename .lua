...
ParticleEmitter.Color=ColorSequence.new(Color3.fromRGB(255,255, 255), Color3.fromRGB(85, 255, 255))
...
Tabs.Info:AddButton({
        ...
        Description="Discord",
        Callback=function()
            setclipboard(tostring("https://discord.gg/yzE6hrKf")) 
        end
})
Tabs.Info:AddButton({
    Title="Chú Roblox",
    Description="Youtube",
    Callback=function()
        setclipboard(tostring("https://www.youtube.com/@churoblox"))
       end
})
Tabs.Info:AddParagraph({
    Title="Phát Triển Bởi",
    Content="Team Chú Roblox"
})
local executorName
...
local AutoMystic = Tabs.Item:AddSection("Lấy Tộc")

local ToggleGhoul = Tabs.Item:AddToggle("ToggleGhoul", {
    Title = "Tự Động Lấy Tộc Quỷ",
    Description = "Đánh Boss, Cày Ectoplasm, Mua Tộc",
    Default = false
})

ToggleGhoul:OnChanged(function(Value)
    AutoGhoul = Value
end)

Options.ToggleGhoul:SetValue(false)

spawn(function()
    while wait() do
        if AutoGhoul then
            pcall(function()
                -- Nếu đã có tộc Ghoul thì dừng lại
                if game.Players.LocalPlayer.Data.Race.Value == "Ghoul" then
                    AutoGhoul = false
                    Notif.New("Bạn đã có tộc Ghoul, dừng auto!")
                    return
                end

                -- Kiểm tra có Hellfire Torch không
                ...
                -- Nếu chưa có Torch, đánh Cursed Captain
                        ...
                        for _, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v.Name == "Cursed Captain" then
                                repeat wait(_G.Fast_Delay)
                                    ...
                                    v.HumanoidRootPart.Transparency = 1
                                    sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                                until v.Humanoid.Health <= 0 or AutoGhoul == false
                            end
                        end
                    else
                        Notif.New("Cursed Captain chưa spawn! Đợi tiếp...")
                    end
                end

                -- Nếu đã có Hellfire Torch, farm Ectoplasm
                local ectoplasmCount = game.Players.LocalPlayer.Data.Ectoplasm.Value
                if ectoplasmCount < 100 then
                    for _, npc in pairs(game.Workspace.Enemies:GetChildren()) do
                        if npc.Name:match("Ship") or npc.Name:match("Crewmate") then
                            repeat wait(_G.Fast_Delay)
                                    AttackNoCoolDown()
                                    AutoHaki()
                                    EquipTool(SelectWeapon)
                                ...
                                sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                                ectoplasmCount = game.Players.LocalPlayer.Data.Ectoplasm.Value
                            until ectoplasmCount >= 100 or not npc.Parent
                        end
                    end
                end

                -- Khi có đủ điều kiện, tự động mua tộc Ghoul
                if hasTorch and ectoplasmCount >= 100 then
                    game:GetService("ReplicatedStorage").Remotes.CommE:FireServer("BuyRace", "Ghoul")
                    Notif.New("Đã mua tộc Ghoul thành công!")
                    AutoGhoul = false
                end
            end)
        end
